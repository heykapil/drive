import { Primitive, query } from '@/service/postgres';
  ...
  try {
    ...
    if (!fileId) {
      return NextResponse.json(
        {
          success: false,
          error: "A 'fileId' is required in the request body.",
        },
        { status: 400 },
      );
    }
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            "At least one valid update field ('rename' or 'liked') must be provided.",
        },
        { status: 400 },
      );
    }
    ...
    if (result.rows.length === 0) {
      return NextResponse.json(
        {
          success: false,
          error: 'File not found within the specified bucket.',
        },
        { status: 404 },
      );
    }
