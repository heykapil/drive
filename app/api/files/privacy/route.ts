...
export async function PATCH(req: NextRequest) {
  try {
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            "Invalid request: 'fileIds' must be a non-empty array and 'isPublic' must be a boolean.",
        },
        { status: 400 },
      );
    }
    ...
    const uniqueNumericFileIds = [
      ...new Set(body.fileIds.map(id => Number(id))),
    ];
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            "Invalid request: 'fileIds' must contain only numbers or numeric strings.",
        },
        { status: 400 },
      );
    }
    ...
    uniqueNumericFileIds.forEach(id =>
      resultsMap.set(id, { fileId: id, status: 'notFound' }),
    );
    ...
    if (successfullyUpdatedIds.length > 0) {
      await query(`UPDATE files SET is_public = $1 WHERE id = ANY($2::int[])`, [
        isPublic,
