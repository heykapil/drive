...
export async function POST(req: NextRequest) {
  try {
    ...
    for (const bucketId of uniqueBucketIds) {
      ...
      if (!config) {
        console.error(`Missing configuration for bucketId: ${bucketId}. Cannot generate URLs for ${filesInBucket.length} files.`);
        ...
        return Promise.all(
          filesInBucket.map(async (file) => {
            try {
              ...
              console.error(`Error generating URL for fileId ${file.id} in bucket ${config.name}:`, error);
              return { fileId: file.id, error: "Failed to generate URL", status: 'error' as const };
            }
          })
        );
      })();
