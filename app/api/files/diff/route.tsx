...
type FolderBucketRow = { bucket_id: number };
type DBFileRow = { key: string; bucket_id: number };
type BucketConfig = { id: number; name: string; endpoint?: string };
type S3Object = { Key?: string; Size?: number; LastModified?: string | Date };
type S3ListResponse = { Contents?: S3Object[]; NextContinuationToken?: string };
type MissingFile = {
  key: string;
  size?: number;
  ...
  try {
    ...
    if (!folderIdParam && !bucketIdParam) {
      return NextResponse.json(
        { error: 'Missing folderId or bucketId' },
        { status: 400 },
      );
    }
    ...
    const { rows: dbFiles } = await query<DBFileRow>(
      `SELECT "key", bucket_id FROM files WHERE bucket_id = ANY($1::int[])`,
      [bucketIds],
    );

    const dbFileKeys = new Set(
      dbFiles.map((f: DBFileRow) => `${f.bucket_id}:${f.key}`),
    );
    ...
    await Promise.all(
      bucketConfigs.map(async config => {
        try {
          ...
          do {
            ...
            const response = (await s3.send(command)) as S3ListResponse;
              ...
              for (const obj of response.Contents) {
                ...
                if (!dbFileKeys.has(compositeKey)) {
                  missingFiles.push({
                    ...
                    url: config.endpoint
                      ? `${config.endpoint}/${config.name}/${obj.Key}`
                      : null, // Construct a rough URL if possible
                  });
                }
              }
            }
          ...
          console.error(
            `Error listing objects for bucket ${config.name}:`,
            message,
          );
    ...
    console.error('Error in diff API:', message);
