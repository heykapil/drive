...
export default function EditBucketPage() {
    ...
    const allBuckets = (nodes: FolderNode[]): any[] => {
        let buckets: any[] = [];
        nodes.forEach(node => {
            buckets = buckets.concat(node.buckets);
            if (node.children) {
    ...
    const form = useForm<MoveBucketValues>({ resolver: zodResolver(moveBucketSchema) });
    ...
    const renderFolderItems = (nodes: FolderNode[], onChange: (...event: any[]) => void) => {
        return nodes.map(folder => {
            ...
            return (
                <DropdownMenuItem key={folder.folder_id} onSelect={() => {
                    onChange(folder.folder_id.toString());
                    setSelectedFolderName(folder.folder_name);
                }}>
                    <FolderOpen className="mr-2 h-4 w-4" />
        ...
        <div className="flex justify-center items-start min-h-screen p-4 md:p-8">
            <Card className="w-full max-w-lg">
                <CardHeader>
                    ...
                    <CardDescription>Select a bucket and its new parent folder.</CardDescription>
                </CardHeader>
                <CardContent>
                    <Form {...form}>
                        <form onSubmit={form.handleSubmit(handleMoveBucket)} className="space-y-6">
                            <FormField control={form.control} name="bucketId" render={({ field }) => (
                                <FormItem>
                                    <FormLabel>Bucket to Move</FormLabel>
                                    <Select onValueChange={field.onChange} defaultValue={!isLoading && selectedBucketId ? selectedBucketId.toString() : field.value}>
                                        <FormControl>
                                        ...
                                        <SelectContent>
                                            {bucketOptions.map(bucket => (
                                                <SelectItem key={bucket.bucket_id} value={bucket.bucket_id.toString()}>
                                                    {bucket.bucket_name}
                                                </SelectItem>
                                            ))}
                                        </SelectContent>
                            ...
                            <FormField control={form.control} name="newFolderId" render={({ field }) => (
                                <FormItem>
