    ...
    try {
        ...
        const uploadChunk = async (partNumber: number, chunk: Uint8Array): Promise<void> => {
            ...
            if (isSynologyBucket) {
                ...
                // Use provided endpoint or default logic
                let url;
                if (endpoint && endpoint !== 'default') {
                    ...
                    url = production
                        ? `${process.env.NEXT_PUBLIC_GCLOUD_URL_CHUNK}/upload?bucket=${selectedBucketId}`
        ...
        // 9. Cleanup resources
        if (currentUploadId) {
            await fetch('/api/upload/clean-up', {
                method: 'POST',
                body: JSON.stringify({ uploadId: currentUploadId }),
            });

            // If endpoint is custom, we might want to clean up there too if it supports it.
            // But for now we only clean up our own backend resources.
            if (endpoint && endpoint !== 'default') {
                try {
                    await axios.post(`${endpoint}/clean-up`, { uploadId: currentUploadId });
                } catch (e) {
                    console.warn("Failed to cleanup on custom worker", e);
                }
            }
        }
