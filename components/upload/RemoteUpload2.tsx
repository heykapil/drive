...
export default function RemoteUpload({
  ...
  const { selectedUniqueId: selectedBucketId, selectedBucketName, isLoading: isZustLoading } = useBucketStore()
  ...
  useEffect(() => {
    async function setSynologyBucket() {
      if (!isZustLoading && selectedBucketId) {
        // Handle unified ID
  ...
  const handleUpload = async (useProxy: boolean, proxyUrl?: string) => {
    ...
    // Parse ID for S3 actions
    let numericId: number | null = null;
    if (selectedBucketId.startsWith('s3_')) {
      numericId = parseInt(selectedBucketId.replace('s3_', ''), 10);
    } else if (!isNaN(parseInt(selectedBucketId, 10))) {
      numericId = parseInt(selectedBucketId, 10);
    }

    if (selectedBucketId.startsWith('tb_')) {
      return toast.error("Terabox uploads are not supported in this view yet.");
    }

    if (numericId === null) return toast.error("Invalid bucket selected");
    ...
    for (const [index, url] of urlList.entries()) {
      toast.promise(uploadMultipart(url, numericId, setProgress, encryptBucketConfigAction, proxy, synologyBucket), {
