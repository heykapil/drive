...
import { isTeraboxUrl } from "@/lib/helpers/isTeraUrl";
...
export default function RemoteUpload() {
  ...
  const handleUpload = async (useProxy: boolean, proxyUrl?: string) => {
    const originalUrls = urls.split("\n").map(url => url.trim()).filter(Boolean);
    if (originalUrls.length === 0) return toast.error("No URLs found");

    const proxy = useProxy ? proxyUrl : undefined;
    setIsUploading(true);

    try {
      // Process all URLs to resolve Terabox links
      const resolvedUrls = await Promise.all(
        originalUrls.map(async (url) => {
          if (isTeraboxUrl(url)) {
            try {
              return await extractTeraboxDownloadUrls(url)
                .then(()=> toast.info('Extracted terabox direct download link'));
            } catch (error: any) {
              toast.error(`Skipping Terabox URL: ${error.message}`);
              return [];
            }
          }
          return [url];
      ...
      const allDownloadUrls = resolvedUrls.flat() as string[];
      if (allDownloadUrls.length === 0) return toast.error("No valid URLs found");
      setProgress(Object.fromEntries(allDownloadUrls.map(url => [url, 0])));

      // Upload all resolved URLs
      for (const [index, url] of allDownloadUrls.entries()) {
        toast.promise(
          uploadMultipart(url, selectedBucket, setProgress, proxy),
          {
            loading: `Uploading... ${index + 1} of ${allDownloadUrls.length}`,
            error: `Error in URL ${index + 1}: ${url}`,
          }
        );
      }
    } catch (error: any) {
