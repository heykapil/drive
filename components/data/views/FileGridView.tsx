  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
      {files.map(file => (
        <div
          key={file.id}
          className={`group relative flex flex-col bg-card border rounded-xl overflow-hidden transition-all duration-300 ${
        ...
        >
            ...
            <div
              className={`w-5 h-5 rounded-md border flex items-center justify-center cursor-pointer ${
                selectedFiles.has(file.id)
                  ...
                  : 'bg-background/80 backdrop-blur border-muted-foreground/30 hover:border-primary'
              }`}
            >
              {selectedFiles.has(file.id) && (
          ...
          <div className="absolute top-1 right-1 z-20">
            {parseInt(file?.quality?.replace('M3U8_AUTO_', ''), 10) > 0 && (
              <Badge className="text-xs">
                {parseInt(file?.quality?.replace('M3U8_AUTO_', ''), 10) + 'p'}
              </Badge>
            )}
          </div>
          ...
          <div className="aspect-[4/3] bg-muted/30 relative overflow-hidden group-hover:bg-muted/50 transition-colors">
            {file.is_public && file.type.startsWith('image/') ? (
              ...
              file.filename?.match(/\.(mp4|webm|ogg|mov)$/) ? (
          ...
          <div className="p-2 flex flex-col gap-1">
            <div className="flex items-start justify-between gap-2">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  ...
                  <DropdownMenuSub>
                    ...
                    <DropdownMenuPortal>
                      <DropdownMenuSubContent>
                        <DropdownMenuItem
                          onClick={() => {
                            const seconds = prompt(
                              'Enter seconds for thumbnail (e.g. 10):',
                              '10',
                            );
                            if (seconds)
                              actions.onUpdateThumbnail(
                                file,
                                parseInt(seconds),
                              );
                          }}
                        ...
                        <DropdownMenuItem
                          onClick={() => actions.onUpdateDuration(file)}
                        >
                          Update Duration
                        </DropdownMenuItem>
                        <DropdownMenuItem
                          onClick={() => actions.onUpdateQuality(file)}
                        >
                          Update Quality
                        </DropdownMenuItem>
                        <DropdownMenuItem
                          onClick={() => actions.onUpdateShareId(file)}
                        >
                          Update Share ID
                        </DropdownMenuItem>
                      </DropdownMenuSubContent>
                  ...
                  <DropdownMenuItem
                    onClick={() => actions.onTogglePrivacy(file)}
                  >
                    {file.is_public ? (
            ...
            <div className="flex items-center justify-between text-xs text-muted-foreground">
              ...
              <span>{file.duration > 0 && formatSeconds(file.duration)}</span>
            </div>
          </div>
        </div>
      ))}
