...
export async function middleware(request: NextRequest) {
  ...
  const session_token =
    request.cookies.get(`${cookiePrefix}kapil.app.session_token`)?.value || '';
  const sessionData =
    request.cookies.get(`${cookiePrefix}kapil.app.sessionData`)?.value || '';
  ...
  if (production && !session_token) {
    return NextResponse.redirect(
      new URL(
        '/login?redirectTo=' + encodeURIComponent(publicCallbackUrl.toString()),
  ...
  if (production && session_token && !sessionData) {
    ...
    if (production && !session) {
      return NextResponse.redirect(
        new URL(
          '/login?redirectTo=' +
            encodeURIComponent(request.nextUrl as unknown as string),
          process.env.BETTER_AUTH_URL!,
        ),
      );
    }
    ...
    if (production) {
      midResponse.cookies.set({
        ...
        sameSite: 'lax',
        domain:
          process.env.NODE_ENV === 'production' ? '.kapil.app' : '.localhost',
